...
contract CarbonCreditDutchAuctionGuardedLaunchTest is Test {
    ...
    function setUp() public {
        ...
        vm.warp(100000);
        glow =
            new TestGLOWGuardedLaunch(earlyLiquidityAddress,vestingContract,SIMON,address(usdg),address(uniswapFactory));
        ...
        minerPoolAndGCA = new MockMinerPoolAndGCA(startingAgents,
        address(glow),governance,keccak256("requirementsHash"),earlyLiquidityAddress,address(usdg),address(vetoCouncil),address(address(holdingContract)));
        // gcc = new TestGCC(address(this), address(this),address(glow),address(usdc),address(uniswapRouter));
        gcc = new TestGCCGuardedLaunch({
            _gcaAndMinerPoolContract: address(minerPoolAndGCA), //no need in this test
            _governance: address(governance),
            _glowToken: address(glow),
            _usdg: address(usdg),
            _vetoCouncilAddress: address(vetoCouncil),// no need in this test
        ...
        vm.startPrank(SIMON);
        glow.setContractAddresses({
            _gcaAndMinerPoolAddress: address(minerPoolAndGCA),
            _vetoCouncilAddress: address(vetoCouncil),
            _grantsTreasuryAddress: address(0xdead)
        });
        ...
        usdg.setAllowlistedContracts({
            _glow: address(glow),
            _gcc: address(gcc),
            _holdingContract: address(address(holdingContract)),
            _vetoCouncilContract: address(governance),
