...
contract MigrationTest is Test {
    ...
    function setUp() public {
        mainnetFork = vm.createFork({urlOrAlias: mainnetForkUrl, blockNumber: 23548497});
    ...
    */
    function test_v2_mainnet_claim() public {
...

        {
            ...
            taa[0] = FoundationRewardKernel.TokenAndAmount({
                token: address(0xe010ec500720bE9EF3F82129E7eD2Ee1FB7955F2), amount: 276838997349
            });
        ...
        FoundationRewardKernel.TokenAndAmount[] memory taa = new FoundationRewardKernel.TokenAndAmount[](1);
        taa[0] = FoundationRewardKernel.TokenAndAmount({
            token: address(0xe010ec500720bE9EF3F82129E7eD2Ee1FB7955F2), amount: 297867047
        });
    ...
    struct Json {
        bytes32 v1MerkleRoot;
        bytes32 v2MerkleRoot;
        uint256 totalGlowInflationRewardsLeafWeight;
        uint256 totalV1UsdgWeight;
        ReadableLeaf[] readableLeaves;
    }

    struct ReadableLeaf {
        bytes32 leaf;
        address user;
        // uint256 glowInflationEarnedLeafWeight;
        // uint256[] v1MerkleProof;
    }

    function test_claimAllV1InV2() public {
        string memory root = vm.projectRoot();
        string memory path = string.concat(root, "/test/migration/data.json");
        string memory _json = vm.readFile(path);
        bytes memory bjson = vm.parseJson(_json);
        Json memory json = abi.decode(bjson, (Json));
        // uint256 readableLeavesLength =
        //log length of json.length
        console2.log("json-length", json.readableLeaves.length);
