...
import {GCASalaryHelperV2 as GCASalaryHelper} from "@/MinerPoolAndGCA/GCASalaryHelperV2.sol";
import {MockGovernance} from "@/testing/MockGovernance.sol";
import {MockSalaryHelperV2 as MockSalaryHelper} from "@/MinerPoolAndGCA/mock/MockSalaryHelperV2.sol";

contract GCAPayoutTestV2 is Test {
    //--------  CONTRACTS ---------//
    MockGCA gca;
    TestGLOW glow;
    Handler handler;
    ...
    function test_v2_integrationSubmittingCompPlans() public {
        ...
        hash = keccak256(abi.encode(gcasToSlash, newGCAs2));
        gca.pushHash(hash, false);
        gca.executeAgainstHash(gcasToSlash, newGCAs2);
    ...
    function test_v2_claimPayout_slashedAgent_claimPayout_shouldRevert() public {
    ...
    function test_v2_claimPayout_activeGCAsAtPaymentNonce_doNotMatch_shouldRevert() public {
    ...
    function test_v2_claimPayout_invalidGCAIndex_shouldRevert() public {
        vm.warp(block.timestamp + 5 weeks);
    ...
    function test_v2_sharesDoNotAddUpTo100_000_shouldRevert() public {
        vm.startPrank(startingGCAs[0]);
        //for the new comp plan, i want to distribute the shares as follows:
        //  1. 50% to me (the first GCA)
        //  2. 25% to the second GCA
    ...
    function test_v2_GCASalaryHelper_allBaseFunctions_shouldRevertWhenNotOverriden() public {
