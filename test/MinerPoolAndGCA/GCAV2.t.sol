...
import {MockGCAV2 as MockGCA} from "@/MinerPoolAndGCA/mock/MockGCAV2.sol";
...
contract GCAV2Test is Test {
    ...
    function test_v2_expectCurrentWeek_toRevert_becauseItIsNotOverriden() public {
        vm.expectRevert();
    ...
    function test_v2_invalidBucketSubmission_initBucket_withDifferentSlashNonce_shouldAlwaysRevert() public {
        uint256 bucketId = 2;
        bucketId = bound(bucketId, 0, 1 * 1e15);
        addGCA(SIMON);
        vm.startPrank(SIMON);
    ...
    function test_v2_issueReport() public {
        issueReport({
            gcaToSubmitAs: SIMON,
            bucket: 0,
    ...
    function test_v2_issueReport_andCheckGlobalState() public {
        test_v2_issueReport();
    ...
    function test_v2_issueReport_newSubmissionShouldOverrideOldOne() public {
        issueReport({
            gcaToSubmitAs: SIMON,
            bucket: 0,
            totalNewGCC: 100 * 1e15,
    ...
    function test_v2_issueReport_newGCAShouldCreateNewReport() public {
    ...
    function test_v2_issueReport_oldReportShouldOverride_whenMultipleReportsInBucket() public {
        //This issues as SIMON in first index
        test_v2_issueReport();

        //This issues as OTHER_GCA in second index
        test_v2_issueReport_newGCAShouldCreateNewReport();
    ...
    function test_v2_issueReport_weightMoreThanUint256Div5_shouldRevert() public {
        addGCA(SIMON);
        vm.startPrank(SIMON);
        uint256 currentBucket = 0;
    ...
    function test_v2_issueReport_moreThan200BillionGCC_shouldRevert() public {
        addGCA(SIMON);
        vm.startPrank(SIMON);
        uint256 currentBucket = 0;
    ...
    function test_v2_issueReport_submittingAfterSubmissionShouldRevert() public {
        test_v2_issueReport();
    ...
    function test_v2_issueReport_submittingBeforeBucketOpenShouldRevert() public {
        addGCA(SIMON);
        vm.startPrank(SIMON);
    ...
    function test_v2_issueReport_notSubmittingToZero_sanityCheck() public {
        uint256 bucketId = 1;
    ...
    function test_v2_issueReport_incrementNonce_shouldStore_ifBucketNotInit() public {
        addGCA(SIMON);
        vm.startPrank(SIMON);
        uint256 currentBucket = 1;
        uint256 totalNewGCC = 101 * 1e15;
    ...
    function test_v2_issueReport_createReport_thenIncrementNonce_shouldClearAllOldReports() public {
    ...
    function test_v2_issueReport_createReport_thenIncrementNonce_shouldClearAllOldReports_newSubmissionShouldPushToArray(
    ) public {
        test_v2_issueReport_createReport_thenIncrementNonce_shouldClearAllOldReports();
        uint256 currentBucket = 0;
    ...
    function test_v2_issueReport_submittingReportLate_shouldRevert_slashNonceDifferent() public {
        addGCA(SIMON);
        vm.startPrank(SIMON);
        uint256 currentBucket = 0;
        uint256 totalNewGCC = 101 * 1e15;
    ...
    function test_v2_issueReport_submittingReportLate_shouldRevert_slashNonceDifferentByTwo() public {
        test_v2_issueReport_submittingAfterSubmissionShouldRevert();
        gca.incrementSlashNonce();
        test_v2_issueReport_submittingReportLate_shouldRevert_slashNonceDifferent();
    }

    function test_v2_incrementNonce() public {
        gca.incrementSlashNonce();
    }

    function test_v2_issueReport_proposalsNotUpdated_shouldRevert() public {
        addGCA(SIMON);
        vm.startPrank(SIMON);
        uint256 currentBucket = 0;
        uint256 totalNewGCC = 101 * 1e15;
    ...
    function test_v2_Constructor_shouldSetGenesisTimestampForGCAs() public {
    ...
    function test_v2_isGCA_shouldReturnTrue_ifGCA() public {
        addGCA(SIMON);
    ...
    function test_v2_isGCA_whenProposalHashesNotUpdated_shouldReturnFalse() public {
        addGCA(SIMON);
        vm.startPrank(SIMON);
        bytes32 proposalHash = keccak256("proposal hash");
        gca.pushRequirementsHashMock(proposalHash);
    ...
    function test_v2_isGCA_whenProposalHashesUpdated_shouldReturnTrue() public {
        addGCA(SIMON);
     ...
     *             - in the glow tests
     */
    function test_v2_claimGlwFromInflation() public {
        vm.warp(block.timestamp + ONE_WEEK);
        gca.claimGlowFromInflation();
        assertTrue(glow.balanceOf(address(gca)) > 0);
    }

    // function test_v2_setGCAs() public {
    ...
    //------------------------ GOVERNANCE CALLS -----------------------------
    function test_v2_setRequirements_callerNotGovernance_shouldFail() public {
        vm.expectRevert(IGCA.CallerNotGovernance.selector);
        gca.setRequirementsHash(bytes32("new hash"));
    }

    function test_v2_setRequirements_callerNotGovernance_shouldWork() public {
        vm.startPrank(governance);
        gca.setRequirementsHash(bytes32("new hash"));
        assertEq(gca.requirementsHash(), bytes32("new hash"));
    }

    function test_v2_pushHash_callerNotGovernance_shouldFail() public {
        bytes32 randomHash = keccak256("new hash");
        vm.expectRevert(IGCA.CallerNotGovernance.selector);
        gca.pushHash(randomHash, true);
    }

    function test_v2_pushHash_callerNotGovernance_shouldWork() public {
        bytes32 randomHash = keccak256("new hash");
    ...
    function test_v2_pushHash_callerNotGovernance_slashNonceShouldNotIncrement() public {
        bytes32 randomHash = keccak256("new hash");
        uint256 slashNonceBefore = gca.slashNonce();
        vm.startPrank(governance);
    ...
    function test_v2_getProposalHashes_pagination() public {
        bytes32 randomHash = keccak256("new hash");
    ...
    function test_v2_executeAgainstHash() public {
    ...
    function test_v2_executeAgainstHash_badInputsShouldRevert() public {
        //Warp to random timestamp
        vm.warp(501);
        uint256 indexOfProposalHash = 0;
        //Start with two GCA's
...
Stashed
 ...
 // function test_v2_getBucketDataEfficient() public {
    //     issueReport(1, SIMON);
    ...
    // function test_v2_getBucketData_gasCheckEfficient() public {
    //     issueReport(1, SIMON);
    //     MockGCA.EfficientBucket memory efficientBucket = gca.getBucketDataEfficient(0);
    // }

    // function test_v2_getBucketDataEfficient_multipleArrays() public {
    //     issueReport(1, SIMON);
