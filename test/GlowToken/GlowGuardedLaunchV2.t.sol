...
import "../../src/testing/GuardedLaunchV2/TestGLOW.GuardedLaunchV2.sol";
...
contract GlowGuardedLaunchV2Test is Test {
    ...
    //-------------------- Contracts --------------------
    TestGLOWGuardedLaunchV2 public glw;
    ...
    function setUp() public {
        ...
        glw = new TestGLOWGuardedLaunchV2(
            EARLY_LIQUIDITY,
            VESTING_CONTRACT,
            address(GCA),
            VETO_COUNCIL,
    ...
    function test_v2_guarded_sendTokensToNotAllowlistedContract_shouldRevert() public {
    ...
    function test_v2_freezeNetwork_notVetoCouncilMember_shouldRevert() public setInflationContracts {
        vm.expectRevert(GlowGuardedLaunch.ErrNotVetoCouncilMember.selector);
        glw.freezeContract();
    }

    function test_v2_freezeNetwork_VetoCouncilMember_shouldWork() public setInflationContracts {
        vm.startPrank(SIMON);
        glw.freezeContract();
        vm.stopPrank();
    }

    function test_v2_networkFrozen_tradesCannotHappen() public setInflationContracts {
        vm.startPrank(SIMON);
        glw.mint(SIMON, 1 ether);
        glw.freezeContract();
    ...
    function test_v2_guarded_Mint() public {
    ...
    function test_v2_guarded_DoubleStake2() public {
        vm.startPrank(SIMON);
        ...
        vm.warp(block.timestamp + TWO_YEARS + 5 minutes);
     ...
     */
    function test_v2_guarded_NewStake()
        public
    ...
    function test_v2_guarded_stakeAndUnstake()
        public
        mintTokens(SIMON, 1e9 ether)
     ...
     */
    function test_v2_guarded_StakeAndUnstake_SinglePosition()
        public
        mintTokens(SIMON, 1e9 ether)
        stakeTokens(SIMON, 1 ether)
        checkNumStaked(SIMON, 1 ether)
    {
        ...
        for (uint256 i; i < unstakedPositions.length; ++i) {
            ...
            assertEq(pos.cooldownEnd, unstakeBlockTimestamp + TWO_YEARS);
            assertEq(pos.amount, 1 ether);
        }
        vm.stopPrank();
    }

    function test_v2_guarded_stake_withEnoughInAnUnstakedPosition_notAtPosiiton0_shouldTrigger_newHead()
        public
        mintTokens(SIMON, 1e9 ether)
        stakeTokens(SIMON, 1 ether)
    ...
    function test_v2_guarded_unstakedPositionsOfPagination_headEqualsTail_emptyPosition_shouldReturnLengthZeroArray()
        public
    {
    ...
    function test_v2_guarded_unstakedPositionsOfPaginations_headEqualTails_notEmptyPosition_shouldReturnLengthOneArray()
        public
        mintTokens(SIMON, 1e9 ether)
        stakeTokens(SIMON, 1 ether)
        unstakeTokens(SIMON, 1 ether)
     ...
     *
     *         -   1. Repeats all steps inside ```test_v2_guarded_StakeAndUnstake_SinglePosition_stakingShouldClaimGLOW``` above.
    ...
    function test_v2_guarded_NewStakeAndUnstake_SinglePosition_stakingShouldClaimGLOW() public {
        test_v2_guarded_StakeAndUnstake_SinglePosition();
        vm.startPrank(SIMON);
        IGlow.UnstakedPosition[] memory unstakedPositions = glw.unstakedPositionsOf(SIMON);
     ...
     */
    function test_v2_guarded_NewStakeAndUnstakeMultiplePositions_allExpired()
        public
     ...
     */
    function test_v2_guarded_StakeAndUnstakeMultiplePositions_noneExpired()
        public
        stageStakeAndUnstakeMultiplePositions
    {
        IGlow.UnstakedPosition[] memory unstakedPositions = glw.unstakedPositionsOf(SIMON);
    ...
    function test_v2_guarded_StakeAndUnstakeMultiplePositions_useAllStakePositions()
        public
        stageStakeAndUnstakeMultiplePositions
    {
        IGlow.UnstakedPosition[] memory unstakedPositions = glw.unstakedPositionsOf(SIMON);
    ...
    function test_v2_guarded_UnstakeOver100ShouldForceCooldown() public {
    ...
    function test_v2_guarded_ClaimZeroTokensShouldFail() public {
        vm.startPrank(SIMON);
    ...
    function test_v2_guarded_ClaimTokens() public {
        vm.startPrank(SIMON);
        glw.mint(SIMON, 1e9 ether);
        glw.stake(1 ether);
    ...
    function test_v2_guarded_ClaimTokens_ClaimableTotalGT_Amount_NoNewTail() public {
        vm.startPrank(SIMON);
        glw.mint(SIMON, 1e9 ether);
        glw.stake(1 ether);
        vm.expectRevert(IGlow.InsufficientClaimableBalance.selector);
    ...
    function test_v2_guarded_ClaimTokens_ClaimableTotalGT_Amount_NewTail() public {
        vm.startPrank(SIMON);
        glw.mint(SIMON, 1e9 ether);
        glw.stake(1 ether);
        vm.expectRevert(IGlow.InsufficientClaimableBalance.selector);
    ...
    function test_v2_setLP_liquidityPool_shouldBeAbleToBeCreated() public {
        seedLP(1 ether, 10 * 1e6);
    }

    function test_v2_guarded_executeSwap_glwUSDG() public {
        seedLP(1 ether, 10 * 1e6);
    ...
    function test_v2_guarded_ClaimInflationFromGCA() public setInflationContracts {
        vm.startPrank(SIMON);
    ...
    function test_v2_guarded_ClaimFromVetoCouncil() public setInflationContracts {
        vm.startPrank(SIMON);
    ...
    function test_v2_guarded_ClaimFromGrantsTreasury() public setInflationContracts {
        vm.startPrank(SIMON);
        ...
        uint256 startingBalance = glw.balanceOf(GRANTS_TREASURY);
    ...
    function test_v2_guarded_UnstakedPositionsOf() public {
        vm.startPrank(SIMON);
        uint256 amountToMint = 1e9 ether;
        glw.mint(SIMON, amountToMint);
        glw.stake(55 ether);
    ...
    function test_v2_guarded_UnstakedPositionsOfPagination() public {
        vm.startPrank(SIMON);
        uint256 amountToMint = 1e9 ether;
        glw.mint(SIMON, amountToMint);
        glw.stake(55 ether);
    ...
    function test_v2_guarded_PaginationTailGreaterThanLengthShouldReturnEmptyArray() public {
        vm.startPrank(SIMON);
        uint256 amountToMint = 1e9 ether;
        glw.mint(SIMON, amountToMint);
        glw.stake(55 ether);
