...
contract OffchainFractionsTest is Test {
    ...
    function test_closeFraction_Success() public {
        ...
        offchainFractions.closeFraction(FRACTION_ID);
    ...
    function testFuzz_buyFractions_StateTransitions(
        ...
        assertEq(stateAfter.buyerStepsPurchased, stateBefore.buyerStepsPurchased + actualStepsBought, "buyerStepsPurchased mismatch");
        
        // Verify fund flows based on minimum shares logic
        if (expectedSoldSteps < minShares) {
            // Below minimum - funds should go to contract
            assertEq(stateAfter.contractBalance, stateBefore.contractBalance + expectedAmount, "contract balance below min");
            assertEq(stateAfter.recipientBalance, stateBefore.recipientBalance, "recipient balance should not change below min");
            assertEq(stateAfter.claimedFromMinShares, stateBefore.claimedFromMinShares, "claimedFromMinShares should not change below min");
        } else {
            ...
            if (!stateBefore.claimedFromMinShares) {
                ...
                assertEq(stateAfter.recipientBalance, stateBefore.recipientBalance + totalAccumulated, "recipient balance first time min");
                ...
                assertEq(stateAfter.recipientBalance, stateBefore.recipientBalance + expectedAmount, "recipient balance after min");
                assertEq(stateAfter.contractBalance, stateBefore.contractBalance, "contract balance after min should not change");
