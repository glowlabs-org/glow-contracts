...
contract GCAPayoutAlgo2 {
    ...
    function submitCompensationPlan(CompensationI[] calldata compensationPlan) public {
        ...
        uint len = oldCompensationPlan.length;
        uint totalSharesToSubtract = len > 0 ? SHARES_REQUIRED_ON_SUBMISSION : 0;
        for(uint i; i<len;++i){
        ...
        uint totalSharesSubmitted;
        for(uint i; i<compensationPlan.length;++i) {
    ...
    function nextRewardMockTimestamp(uint mockTimestamp,address gca) external view returns(uint256) {
        Helper storage helper = _helpers[gca];
        if(!helper.isGCA) revert NotAnAgent();
    ...
    function editSeats(address[] calldata oldAccounts, address[] calldata newAccounts) external {
        for(uint i; i<oldAccounts.length;++i) {
        ...
        for(uint i; i<newAccounts.length;++i) {
