...
//     LibBitmap.Bitmap internal _bucketRequestedResubmissionBitmap;
...
//         (bool isFinalized,,,,) = isBucketFinalizedWithFinalizationTimestamp(bucketId);
//         return isFinalized;
//     }

//     function isBucketFinalizedWithFinalizationTimestamp(uint256 bucketId)
//         public
//         view
//         returns (
//             bool, /*isFinalized*/
//             uint256, /*finalizationTimestamp*/
//             uint256, /*originalSlashNonce*/
//             uint256, /*lastUpdatedSlashNonce*/
//             uint256 /*slashNonceInStorage*/
//         )
//     {
//         uint256 packedData;
...
//         uint256 originalLastNonce = packedData & _UINT64_MASK;
...
//         return (
//             _isBucketFinalized(bucketId, bucketLastUpdatedNonce, finalizationTimestamp, _slashNonce),
//             finalizationTimestamp,
//             originalLastNonce,
//             bucketLastUpdatedNonce,
//             _slashNonce
//         );
//     }
...
//                     bucketId, bucket.originalNonce, lastUpdatedNonce, _slashNonce, bucketFinalizationTimestamp, true
//                 );
...
//         uint256 finalizationTimestamp = _delayOrResubmissionChecks(bucketId, false);

//         _buckets[bucketId].finalizationTimestamp = SafeCast.toUint128(finalizationTimestamp + RESUBMISSION_DELAY);
//         emit IGCA.BucketResubmissionRequested(bucketId);
//     }

//     function _handleBucketDelay(uint256 bucketId) internal {
//         uint256 finalizationTimestamp = _delayOrResubmissionChecks(bucketId, true);
//         _buckets[bucketId].finalizationTimestamp = SafeCast.toUint128(finalizationTimestamp + bucketDelayDuration());
//         emit IGCA.BucketDelayed(bucketId);
//     }

//     function _delayOrResubmissionChecks(uint256 bucketId, bool trueIfFromDelayFalseForResubmissionRequest)
//         internal
//         view
//         returns (uint256 /*finalizationTimestamp*/ )
//     {
//         (
//             bool isFinalized,
//             uint256 finalizationTimestamp,
//             uint256 bucketOriginalSlashNonce,
//             uint256 bucketLastUpdatedSlashNonce,
//             uint256 slashNonceInStorage
//         ) = isBucketFinalizedWithFinalizationTimestamp(bucketId);
//         if (isFinalized) {
//             _revert(IGCA.BucketAlreadyFinalized.selector);
//         }

//         // This function has a check on the parent function that `hasBeenRequestedForResubmissionForMatchingBucketAndSlashNoncePair` must be false
//         uint256 bucketSubmissionEndTimestamp = _calculateBucketSubmissionEndTimestamp({
//             bucketId: bucketId,
//             bucketOriginNonce: bucketOriginalSlashNonce,
//             bucketLastUpdatedNonce: bucketLastUpdatedSlashNonce,
//             _slashNonce: slashNonceInStorage,
//             bucketFinalizationTimestamp: finalizationTimestamp,
//             computeHasBeenRequestedForResubmissionForMatchingBucketAndSlashNoncePair: trueIfFromDelayFalseForResubmissionRequest
//         });
...
//         // If finalization timestamp is zero because the bucket has never been initialized
//         // That means the finalization timestamp should actually be `bucketFinalizationTimestampNotReinstated(bucketId)`
//         if (finalizationTimestamp == 0) {
//             finalizationTimestamp = bucketFinalizationTimestampNotReinstated(bucketId);
//         }
...
//      * @param _slashNonce - the current slash nonce
//      * @param bucketFinalizationTimestamp - the finalization timestamp of the bucket
//      * @dev this function is used to calculate the bucket submission end timestamp
...
//      *     - if the slash event occurs after the bucket has been finalized, the bucket submission start end will be shifted to the WCEIL() of the slash nonce
...
//         uint256 _slashNonce,
//         uint256 bucketFinalizationTimestamp,
//         bool computeHasBeenRequestedForResubmissionForMatchingBucketAndSlashNoncePair
//     ) internal view returns (uint256) {
//         // If the bucket has never been initialized,delayed, or resubmitted
//         if (bucketFinalizationTimestamp == 0) {
//             return bucketEndSubmissionTimestampNotReinstated(bucketId);
//         }
//         // If the bucket has been initialized, and it matches the slash nonce
//         // Then the end submission time does not change from the above
//         if (bucketOriginNonce == _slashNonce) {
//             uint256 ts = bucketEndSubmissionTimestampNotReinstated(bucketId);
//             bool hasBeenRequestedForResubmissionForMatchingBucketAndSlashNoncePair =
//             computeHasBeenRequestedForResubmissionForMatchingBucketAndSlashNoncePair
//                 ? hasBucketBeenRequestedForResubmissionAtSlashNonce(_slashNonce, bucketId)
//                 : false;
//             if (hasBeenRequestedForResubmissionForMatchingBucketAndSlashNoncePair) {
//                 ts += RESUBMISSION_DELAY;
//             }
//             return ts;
//         }
//         // If the lastUpdatedNonce is not equal to the originalNonce,
//         // That means that a slash event occurred.

//         // If the `bucketLastUpdatedNonce` is equal to the `_slashNonce`
//         // That means that the bucket reports were reinstated to match the slash nonce
//         if (bucketLastUpdatedNonce == _slashNonce) {
//             uint256 wceil = _WCEIL(_slashNonce - 1);
//             bool hasBeenRequestedForResubmissionForMatchingBucketAndSlashNoncePair =
//             computeHasBeenRequestedForResubmissionForMatchingBucketAndSlashNoncePair
//                 ? hasBucketBeenRequestedForResubmissionAtSlashNonce(_slashNonce, bucketId)
//                 : false;
//             if (hasBeenRequestedForResubmissionForMatchingBucketAndSlashNoncePair) {
//                 wceil += RESUBMISSION_DELAY;
//             }
//             return wceil;
//         }
//         uint256 bucketSubmissionStartTimestamp = bucketStartSubmissionTimestampNotReinstated(bucketId);
//         //If the slash occurred between the start of the submission period and the bucket finalization timestamp
//         uint256 i = bucketLastUpdatedNonce;
//         console2.log("Starting i = ", i);
//         console2.log("Slash Nonce = ", _slashNonce);
//         //Also log the finalization timestamp
//         console2.log("Bucket finalization timestamp = ", bucketFinalizationTimestamp);
//         for (; i < _slashNonce;) {
//             //log the slashNonceToSlash Timestamp
//             console2.log("Slash Nonce to Slash Timestamp = ", slashNonceToSlashTimestamp[i]);
//             //Log the bucket submission start timestamp
//             console2.log("Bucket Submission Start Timestamp = ", bucketSubmissionStartTimestamp);
...
//         //If i is not zero, that means
//         if (i != 0) {
//             console2.log(" i = ", i);
//             bool hasBeenRequestedForResubmissionForMatchingBucketAndSlashNoncePair =
//             computeHasBeenRequestedForResubmissionForMatchingBucketAndSlashNoncePair
//                 ? hasBucketBeenRequestedForResubmissionAtSlashNonce(i - 1, bucketId)
//                 : false;

//             if (hasBeenRequestedForResubmissionForMatchingBucketAndSlashNoncePair) {
//                 bucketSubmissionStartTimestamp += RESUBMISSION_DELAY;
//             }
//         }
//         //Note: This value has been mutated to represent the end of the submission period
