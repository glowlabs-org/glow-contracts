...
contract BucketSubmissionV2 {
     ...
     *         -   and the first bucket that the token was deposited to
     *         -   and the last bucket that the token was deposited to
     */
    mapping(address erc20 => BucketTracker) internal bucketTracker;

    /* -------------------------------------------------------------------------- */
    /*                                   mappings                                  */
    /* -------------------------------------------------------------------------- */
    /// @notice mappings bucketId -> WeeklyReward
    mapping(address erc20 => mapping(uint256 => WeeklyReward)) internal rewards;
     ...
     */
    function getBucketTracker(address token) external view returns (BucketTracker memory) {
        return bucketTracker[token];
    }
     ...
     */
    function reward(address token, uint256 id) public view returns (WeeklyReward memory) {
        (WeeklyReward memory bucket,) = _rewardWithNeedsInitializing(token, id);
        return bucket;
    }
    ...
    function _addToCurrentBucket(address token, uint256 amount) internal {
        ...
        WeeklyReward memory currentWeeklyReward = rewards[token][bucketToAddTo];
        ...
        if (currentWeeklyReward.inheritedFromLastWeek) {
            rewards[token][bucketToAddTo].amountInBucket += amountToAddOrSubtract;
        ...
        if (!pastDataIrrelevant) {
            ...
            for (uint256 i = lastUpdatedBucket; i < bucketToAddTo; ++i) {
                totalToDeductFromBucket += rewards[token][i].amountToDeduct;
            }
        } else {
         ...
         */
        rewards[token][bucketToAddTo] = WeeklyReward({
            inheritedFromLastWeek: true,
            amountInBucket: (lastBucket.amountInBucket + amountToAddOrSubtract) - totalToDeductFromBucket,
            amountToDeduct: 0
        });
     ...
     */
    function _getAmountForTokenAndInitIfNot(address token, uint256 id) internal returns (uint256) {
        (WeeklyReward memory weeklyReward, bool needsInitializing) = _rewardWithNeedsInitializing(token, id);
        if (needsInitializing) {
            weeklyReward.inheritedFromLastWeek = true;
            weeklyReward.amountToDeduct = 0;
            rewards[token][id] = weeklyReward;
        }
     ...
     */
    function _rewardWithNeedsInitializing(address token, uint256 id) private view returns (WeeklyReward memory, bool) {
